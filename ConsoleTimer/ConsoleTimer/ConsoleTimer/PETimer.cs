...
public class PETimer{
    ...
    #region TimeTask
    public int AddTimeTask(Action<int> callback, double delay, int count = 1, PETimeUnit timeUnit = PETimeUnit.Millisecond) {
    ...
    public bool ReplaceTimeTask(int tid, Action<int> callback, float delay, int count = 1, PETimeUnit timeUnit = PETimeUnit.Millisecond) {
        if (timeUnit != PETimeUnit.Millisecond) {
            switch (timeUnit) {
                case PETimeUnit.Second:
                    delay = delay * 1000;
